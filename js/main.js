...
if (id == 'done') {
	$('#result-msg').text('Thank you for your details.')
	updateURL('');
};
(function () {
	...
	var sliderSayings = function () {
		$('#fh5co-sayings .flexslider').flexslider({
			...
			after: function () {
				if (count == 2) {
					$('html, body').animate({
						scrollTop: parseInt($("#arrow").offset().top)
					}, 2000);
				}
				count++;
			},
	...
	$('form#contactForm').submit(function (e) {
		e.preventDefault();
		var values = '';
		$('.error').removeClass('focus-border');
		var error_flag = 0;

		var name = $('input[name="name"').val().trim();
		var message = $("#message").val().trim();
		var phone = $('input[name="contact"').val().trim();
		var occation = $('#occation').val();
		var date = $('input[name="date"').val();
		var email = $('input[name="email"').val().trim();

		if (!name || !isValidName(name)) {
			error_flag = 1;
			$('input[name="name"').addClass('focus-border');
		}

		if (!email || !isValidEmailAddress(email)) {
			error_flag = 1;
			$('input[name="email"').addClass('focus-border');
		}

		if (!phone || !isValidPhone(phone)) {
			error_flag = 1;
			$('input[name="contact"').addClass('focus-border');
		}

		if (!occation) {
			error_flag = 1;
			$('#occation').addClass('focus-border');
		}

		if (!date) {
			error_flag = 1;
			$('#date').addClass('focus-border');
		}

		if (!message) {
			error_flag = 1;
			$("#message").addClass('focus-border');
		}
		values = 'name=' + name + '&email=' + email + '&contact=' + phone + '&occation=' + occation + '&date=' + date + '&message=' + message + '&submit=submit';
		if (error_flag == 1)
			return false;
		else {
			$.ajax({
				url: 'lead_form_submit.php',
				type: 'POST',
				data: values,
				success: function (data) {
					if (data == 'done') {
						...
						$('#result-msg').text('Could not submit your details.');
					}
...
function updateURL(param_string) {
	if (history.pushState) {
		var newurl = window.location.protocol + "//" + window.location.host + window.location.pathname + "?" + param_string;
